...
my_palette <- colorNumeric(palette = "viridis"
                  , domain =  playerworldranking$avgpickvalue)
...
leaflet(data = playerworldranking) %>% 
  ...
  addPolygons(
    ...
    addLegend(pal = my_palette,
           ...
           position = "topright",
           title = "Goalie Value In NHL <br> (Successful Players)", opacity = 1)
...
```{r, out.width="80%", fig.align="center"}
knitr::include_graphics("pictures/GOALIEGIFDS.gif")
```
...
```{r, out.width="80%", fig.align="center"}
knitr::include_graphics("pictures/1210663154.0.jpg")
```


The NFL draft process occurs when NFL teams pick players, often attending and playing in colleges and universities across the country. The pattern that most typically occurs is the best college football players in the country are often picked in the first couple of rounds. For example, the first player picked in the first round of the draft is likely very talented and highly sought-after. A player’s performance at their college or university is an important consideration for how likely, or quickly they will be picked by an NFL team. Something to consider when looking at players drafted in the NFL, and their corresponding college, is whether certain college football teams produce some of the first picks in the NFL draft. Throughout this analysis we can determine which U.S. states produce some of the top overall NFL draft picks over fifteen years. This information can help us identify states with the top college football players, as well as determine if there are any correlations between the state a player went to school in and the NFL team they were drafted to. 

The data below represents the NFL draft process over fifteen years (2006 to 2021). Included in this data including players' names, the school they attended, the NFL team they were drafted to, the overall pick of each player (ranging from 1-32), and the year players were drafted. The map below specifically looks at players' colleges and universities in states across the country and the players' corresponding overall pick number. 
...
Before analyzing this topic, we hypothesized that states such as Alabama, Texas, and Louisiana would have some of the highest overall draft picks. I thought this based on the stigma around large football schools in these states, and how seemingly successful they have been over the past few years. However, after using spatial visualizations to further analyze this data we found that states such as New York, Oregon, Wyoming, and Utah have had college football players drafted in the first round and had overall picks of ten or higher, over the period from 2006 to 2021. On the map provided above, these states are colored in red and orange, indicating a player picked relatively high in the first round. Something to consider when looking at this data is that the map only shows data for the first round of the NFL draft when in reality, there are seven rounds total. If we were to alter the data and look at different rounds, we might see different results. Populations that could find these results interesting are high school recruits whose goal is to make it to the NFL. They could look at the map, or use the search function in the table to see which college or university has consistently sent players to the NFL. To personalize these results further, high school football recruits could search their position and determine what school, and NFL team, previous players have gone to.

Another aspect of the map to consider are the states colored in gray. The gray-colored states represent states for which there are no colleges that sent players to the first round of the NFL draft from the years 2006 to 2021. We can see there is a concentration of these states in the far northeast portion of the map, considerably the states Maine, Vermont, New Hampshire, and Rhode Island. It is widely assumed and stereotyped that some of the best college football schools are located in the southern and mid-Atlantic portion of the U.S. From these results we can confirm this assumption and determine that some of the most northern states in the country are not as successful, or do not have as talented players, attending school in these states. 


# NBA

The NBA Draft is an annual event where the 30 NBA teams in the U.S. select players to join their rosters. These players usually come from college basketball programs, international leagues, or high school graduates. The drafting order is determined by a combination of regular-season performance and a lottery system. For instance, teams that perform poorly during the regular season have a higher chance of securing a top pick in the draft, while successful teams pick later.

I used a dataset of the career stats of NBA players from the last 20 NBA Drafts. The dataset included information about the players, teams, and draft picks. Additionally, it also included various attributes, such as the players' names, positions, college or international team affiliations, and pre-draft statistics like points per game, rebounds, and assists. Furthermore, the dataset may include details about the draft itself, like the year, round, and overall pick number. By analyzing this dataset, I aimed to uncover trends, evaluate players, and gain insights into the drafting process and its impact on team performance and player development. This valuable information can help teams make informed decisions in future drafts and enable the general public to better understand the intricacies of the NBA Draft.

I used a spatial map to represent the average points of NBA players that display the geographical distribution of player scoring averages based on their origins. This map typically plots the players' state team, with color-coding indicating their average points scored per game throughout their careers.
...
I used a spatial map representing the value pick of NBA players that displays the geographical distribution of drafted players based on their value or success in the league. This map typically plots the players' hometeam, with color-coding indicating their relative value or performance.

This spatial map helps identify patterns in the distribution of high-value players across different regions. It can reveal potential hotspots for player development, shed light on the effectiveness of college programs and international leagues in producing top talent, and provide insights into the global reach of the NBA. By analyzing this spatial representation, teams, scouts, and analysts can better target their scouting efforts, focus on regions with a high density of valuable players, and potentially uncover untapped talent
...
I created an interactive table displaying the top 3 value picks from each state. It is a dynamic, user-friendly tool that allows users to explore and compare the best-performing NBA players hailing from different states. This table typically presents columns including player name, state, team, and key performance metrics like valuepick and other advanced statistics.

```{r echo=FALSE, message=FALSE}
#creating an interactive table to display the top 3 value picks from each State
...
final_valuepicks_display <- top_valuepicks_print[, c("Pick", "Franchise", "Player", "College", "NBA Team(s)", "State", "valuepick")]%>%
  rename(NBA_Team = 'NBA Team(s)')%>%
  mutate(across(is.numeric, round, digits = 3))
final_valuepicks_display%>%
  select(Player, Franchise, Pick, College, NBA_Team, State, valuepick) %>% 
  datatable(colnames = c("Name", "Position", "Overall", "School", "Year"),
            rownames = head(final_valuepicks_display),
            filter = 'top',
            options = list(pageLength = 10, autoWidth = TRUE))
```
## Analysis

An analysis of a spatial map representing the value pick and average points scored of NBA players helps provide valuable insights into the geographical distribution of player performance. We can use the spatial map to analyze the following trends:

Regional Patterns: Observe any regional trends or hotspots for high-value picks and prolific scorers. Are certain areas consistently producing top-tier talent or high-scoring players? This information can help teams target their scouting efforts and identify regions with strong basketball programs or player development systems. We can see that Colorado, Louisiana, and Ohio have the highest average number of points scored by NBA players.

College and International Influence: Examine the role of college programs and international leagues in player development. Are certain colleges or countries more successful in producing high-value picks or high-scoring players? This can inform teams' scouting strategies and highlight the effectiveness of specific programs or leagues. The table displays the top three value picks from every state along with the college program they were enrolled in.

Comparison Between Value Pick and Scoring Ability: Investigate the relationship between a player's value pick and their scoring ability. Are high-value players generally also high scorers, or do some regions produce high-value picks with diverse skill sets? We can see that NBA players who play for Oklahoma, Ohio, Colorado, and Indiana have the highest value pick compared to other states in the US.   

# Final Thoughts Of Results
Based on exploring the three different sports, each sport produces different results and each sport is hard to compare based on the process that each is conducted. Each sport has its own intricacies that you have to take into account. For example, in the NBA and NFL players have to declare to be included in the draft while the NHL can choose anyone regardless if they declare or not.

Each observation we conducted was particularly interesting and applies to the casual fan and the front office of each of the major sports teams. Here are some of the interesting findings for each sport:

NHL

- As an overall body of work, countries like the United States and Canada may not produce as valuable of picks as countries in Europe. This should show general managers in the NHL that they should start taking more chances on players overseas that may have a chance to pan out even if they have to wait a few years to get them because many end up becoming impactful players that bolster franchises.

- Owners should look at whether or not their general managers are making useful picks in the draft. If a team is consistently not making a long playoff run they should look whether the players they drafted match up against the other teams across the league. Because based on the data, teams that have been at the bottom of the standings consistently and have not won a championship sometimes tend to be drafting poorly. Teams can use this data to determine where to look for players that will perform at the highest level on average.

NFL

- General managers can focus their attention more towards the schools in New York, Oregon, Wyoming, and Utah when recruiting players because these states have historically produced the most first round draft picks. This can save these front office members time in the long run because these schools on average will produce the most players.

- Front office members can stay away from the upper Northeast region because there has not been any players that have been drafted in the first round and if they are looking for impact players they can save time and look elsewhere.

NBA

- By carefully analyzing a spatial map representing the value pick and average points scored of NBA players, teams, scouts, and analysts can uncover valuable insights into player origins, development, and performance. Additionally, such an analysis can contribute to a better understanding of regional influences on basketball styles and help reveal patterns and trends in the development of high-value NBA talent.

Even though this data is useful there are some limitations to the findings. For example, it is hard to judge the intangibles of a player from this data. It is difficult to generalize that if historically players that originate from a certain region pan out that they will continue. This is because there aren't always the same coaches in the regions and the training may change over the years so new powerhouses can emerge. Also, you cannot take all of these findings literally. Just because on average some places may not produce the best valued picks there are always generational talents that emerge. For example, in the NHL dataset Connor Mcdavid who is currently the best player in the NHL by a wide margin is not at the top of the rankings for being the best valued. This is because he was the number one overall pick and does not get the same leniency that a player that was picked 100th and is a consistent player. It doesn’t mean that the Edmonton Oilers should have passed on a generational talent to get a player that they were unsure of. 

Our study could be improved in multiple ways, we only examined a few variables and how they interacted with each other. There are many more factors that occur in the overall value of a player more than just points or win shares. General managers have to look at more to evaluate a player when bringing them into a franchise. But some of these factors like attitude have no way of being measured and trying to find a metric for a player and how they interact and support teammates could be a movement to a stat that would be useful.
...
<!-- Dataset -->
- OP, Matt (September 2022), "NHL Draft Hockey Player Data (1963 - 2022)", *Kaggle*, available at <https://www.kaggle.com/datasets/mattop/nhl-draft-hockey-player-data-1963-2022>.

<!-- Dataset -->
- Lichtenstein, Jack (January 2021), "ESPN NFL draft prospect data" (Version 1), *Kaggle*, available at <https://www.kaggle.com/datasets/jacklichtenstein/espn-nfl-draft-prospect-data?select=nfl_draft_prospects.csv>.

<!-- Dataset -->
- Wieland, Ben (April 2022), "20 Years of NBA Draft Data" (Version 1), *Kaggle*, available at <https://www.kaggle.com/datasets/benwieland/nba-draft-data>.

<!-- Website -->
- Hockey References, "Point Share Calculations" available at <https://www.hockey-reference.com/about/point_shares.html>.
...
<!-- R package -->
- Deckmyn, Alex (2022). "maps: Draw Geographical Maps," R package version 3.4.1, available at <https://cran.r-project.org/web/packages/maps/index.html>.

<!-- R package -->
- Cheng Joe (2023). "leaflet: Create Interactive Web Maps with the JavaScript 'Leaflet' Library," R package version 2.1.2, available at <https://cran.r-project.org/web/packages/leaflet/index.html>.


<!-- R package -->
- Wickham H, François R, Henry L, Müller K, Vaughan D (2023). dplyr: A Grammar of Data Manipulation. R package version 1.1.0, available at  <https://CRAN.R-project.org/package=dplyr>.

<!-- R package -->
- Hughes E (2022). tidytuesdayR: Access the Weekly ‘TidyTuesday’ Project Dataset. R package version 1.0.2, available at <https://CRAN.R-project.org/package=tidytuesdayR>.
